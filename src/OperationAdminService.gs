function listOperationsForAdmin_(query) {
  requireAdmin_(true);
  query = query || {};
...
function getOperationDetailForAdmin_(operationId) {
  requireAdmin_(true);
...
function reconcileOperationForAdmin_(operationId) {
  ...
  if (operation.action === 'EDIT_ASSET') {
    ...
    result = updateEquipment_(equipmentUpdateInput);
  }
  ...
  if (operation.action === 'CREATE_USER') {
    result = createUser_(mergeObjects_(payload, { command_id: commandId }));
  }
  ...
  if (operation.action === 'CREATE_CATEGORY') {
    result = createCategory_(mergeObjects_(payload, { command_id: commandId }));
  }
  ...
  if (operation.action === 'UPLOAD_ASSET_IMAGE') {
    result = reconcileImageOperationForAdmin_(commandId);
  }
...
function abortOperationForAdmin_(input) {
  input = input || {};
  ...
  return withAdminMutation_(function (actor) {
...
function reconcileImageOperationForAdmin_(operationId) {
  return withAdminMutation_(function (actor) {
    var operation = findRecordById_(SHEETS.OPERATIONS, 'operation_id', operationId);
    ...
    if (persistedHistory) {
      ...
      ensureOperationHistoryLocked_(equipmentImageHistoryEntry_(
        before, current, operation, 'Uploaded equipment image'), actor);
...
function reconcileAutoProvisionOperationForAdmin_(operationId) {
  return withAdminMutation_(function (actor) {
    var operation = findRecordById_(SHEETS.OPERATIONS, 'operation_id', operationId);
    assertApp_(operation && operation.status === OPERATION_STATUS.STARTED &&
